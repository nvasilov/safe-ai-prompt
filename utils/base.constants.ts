export const STORAGE_PREFIX = 'local:'

export const EMAIL_REGEX = /[A-Z0-9._-]+@[A-Z.-]+\.[A-Z]{2,}/gi
export const EMAIL_PLACEHOLDER = '[EMAIL_ADDRESS]'

export const CHATGPT_URL_MATCH = "https://chatgpt.com/*"
export const CHATGPT_URL_REGEX = /^https:\/\/chatgpt\.com\/backend(-anon|-api)?\/f\/conversation$/

export const WORLD_TO_ISOLATED_NS = "world_to_isolated_ns"

export const SANITIZE_CHATGPT_REQUEST_PAYLOAD_MSG = "sanitize_chatgpt_request_payload_msg"

export const MOMENT_FULL_FORMAT = "HH:mm:ss / DD MMM YYYY"
export const MOMENT_TIME_FORMAT = "HH:mm:ss"